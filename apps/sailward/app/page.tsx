...
export default function Home() {
  ...
  const [settingsOpen, setSettingsOpen] = useState(false); const [toolsOpen, setToolsOpen] = useState(false); const [capturingKey, setCapturingKey] = useState<KeyBindingName | null>(null); const [keyBindings, setKeyBindings] = useState<KeyBindings>(DEFAULT_KEY_BINDINGS);
  ...
  useEffect(() => { const key = "sailward.inventoryFolders.v1"; const render = () => { const inventory = document.querySelector<HTMLElement>(".tools-inventory"); if (!inventory || inventory.querySelector(".inventory-personal")) return; const personal = document.createElement("section"); personal.className = "inventory-personal is-open"; personal.innerHTML = "<b>CARPETAS PERSONALES</b><div></div>"; const add = document.createElement("button"); add.className = "inventory-add-folder"; add.textContent = "+ NUEVA CARPETA"; add.onclick = () => { const name = window.prompt("Nombre de la carpeta"); if (!name?.trim()) return; const folders = JSON.parse(window.localStorage.getItem(key) ?? "[]") as string[]; folders.push(name.trim().slice(0, 28)); window.localStorage.setItem(key, JSON.stringify(folders)); personal.remove(); render(); }; const list = personal.querySelector("div")!; const folders = JSON.parse(window.localStorage.getItem(key) ?? "[]") as string[]; folders.forEach((name) => { const row = document.createElement("button"); row.className = "inventory-folder"; row.textContent = `▸ ${name}  ×`; row.onclick = () => { if (!window.confirm(`Eliminar ${name}?`)) return; window.localStorage.setItem(key, JSON.stringify(folders.filter((item) => item !== name))); personal.remove(); render(); }; list.append(row); }); personal.append(add); inventory.append(personal); }; render(); const observer = new MutationObserver(render); observer.observe(document.body, { childList:true, subtree:true }); return () => observer.disconnect(); }, []);
  useEffect(() => {
