...
// PRESIÃ“N DE ACEITE is intentionally moved out of the compact engine control.
...
export default function Home() {
  ...
  useEffect(() => { const syncInventoryChecks = () => { const next: Record<string, boolean> = {}; document.querySelectorAll<HTMLElement>("[data-floating-panel]").forEach((panel) => { if (panel.dataset.floatingPanel) next[panel.dataset.floatingPanel] = !panel.classList.contains("is-tool-closed"); }); setInventoryToolOpen(next); }; syncInventoryChecks(); const observer = new MutationObserver(syncInventoryChecks); observer.observe(document.body, { attributes: true, attributeFilter: ["class"], subtree: true }); return () => observer.disconnect(); }, [voyage]);
  ...
  const toggleInventoryTool = (panel: string) => { const target = document.querySelector<HTMLElement>(`[data-floating-panel="${panel}"]`); if (!target) return; const opening = target.classList.contains("is-tool-closed"); target.classList.toggle("is-tool-closed", !opening); setInventoryToolOpen((current) => ({ ...current, [panel]: opening })); };
  return <main className={`game-shell ${voyage ? "is-sailing" : "is-docked"}`}><div ref={mapContainerRef} className="world-map" aria-label="Mapa mundial interactivo de Sailward" /><div className="ocean-vignette" aria-hidden="true" />
    ...
    {voyage && <>
      ...
      <section className={`instrument-window engine-window floating-window ${minimizedPanels.engine ? "is-minimized" : ""}`} data-floating-panel="engine" style={floatingPanelStyle("engine")} onPointerDownCapture={() => setActivePanel("engine")}>
        ...
        <div className="engine-window-content floating-window-content"><div className="engine-state"><span className={voyage.engineRunning ? "is-running" : ""} /> <strong>{voyage.engineRunning ? "ENCENDIDO" : "APAGADO"}</strong><button type="button" disabled={!voyage.engineRunning && voyage.fuelL <= 0} onClick={() => updateVoyage((current) => ({ ...current, engineRunning: !current.engineRunning && current.fuelL > 0 }))}>{voyage.engineRunning ? "APAGAR" : voyage.fuelL > 0 ? "ENCENDER" : "SIN COMBUSTIBLE"}</button></div><label className={`engine-throttle ${engineGear === "REVERSA" ? "is-reverse" : ""}`}><span>{engineGear}</span><strong>{engineGear === "REVERSA" ? "−" : ""}{Math.abs(Math.round(voyage.motor))}%</strong><input aria-label="Acelerador del motor con reversa" type="range" min="-100" max="100" value={voyage.motor} disabled={!voyage.engineRunning} onChange={(event) => updateVoyage((current) => ({ ...current, motor: Number(event.target.value) }))} /></label></div>
      </section>
