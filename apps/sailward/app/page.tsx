...
export default function Home() {
  ...
  const [hydrated, setHydrated] = useState(false); const [selectedPortId, setSelectedPortId] = useState(PORTS[0].id); const [voyage, setVoyage] = useState<Voyage | null>(null);
  ...
  const updateVoyage = useCallback((change: (current: Voyage) => Voyage) => setVoyage((current) => current ? change(advanceVoyage(current, conditionsRef.current, Date.now())) : current), []);
  ...
  useEffect(() => {
    ...
    void import("maplibre-gl").then(({ default: maplibregl }) => { if (disposed || !mapContainerRef.current) return; const map = new maplibregl.Map({ container: mapContainerRef.current, style: BASE_STYLE, center: [selectedPort.lon, selectedPort.lat], zoom: 8.2, pitch: 56, bearing: -34, maxPitch: 70, attributionControl: false });
  ...
  return <main className={`game-shell ${voyage ? "is-sailing" : "is-docked"}`}><div ref={mapContainerRef} className="world-map" aria-label="Mapa mundial interactivo de Sailward" /><div className="ocean-vignette" aria-hidden="true" />
    <header className="brand-bar"><div className="brand-lockup"><span className="brand-mark" aria-hidden="true">S</span><div><strong>SAILWARD <em>v{APP_VERSION}</em></strong><span>REAL-TIME SAILING</span></div></div><div className="world-clock"><span className="live-dot" /><span>{utcTime} UTC</span><small>TIEMPO REAL · 1×</small></div></header>
    ...
    {hydrated && !voyage && <section className="departure-panel"><span className="eyebrow">PUERTO DE PARTIDA</span><h1>El mar ya está en movimiento.</h1><p className="departure-copy">Elegí un puerto real. El timón gobierna el giro; el barco responde a las velas, el viento y la corriente.</p><label className="field-label" htmlFor="port">Zarpar desde</label><div className="port-select-wrap"><select id="port" value={selectedPortId} onChange={(event) => setSelectedPortId(event.target.value)}>{PORTS.map((port) => <option key={port.id} value={port.id}>{port.name}, {port.country}</option>)}</select></div><div className="departure-weather"><div><span>VIENTO</span><strong>{conditions.windKn.toFixed(0)} kn</strong></div><div><span>OLAS</span><strong>{conditions.waveHeight.toFixed(1)} m</strong></div><div><span>FONDO</span><strong>~{conditions.depthM} m</strong></div></div><button className="primary-action" onClick={startVoyage}><span>INICIAR TRAVESÍA</span><span>→</span></button><div className="feature-row"><span>Viento real</span><span>Polars</span><span>Cartografía náutica</span></div></section>}
    {voyage && <><aside className="voyage-card"><span>TRAVESÍA ACTIVA</span><strong>{PORTS.find((port) => port.id === voyage.portId)?.name}</strong><span>{voyage.autopilot ? `PILOTO · ${compassPoint(voyage.targetHeading ?? voyage.heading)}` : "MANUAL · TIMÓN"}</span><div className="voyage-distance">{voyage.distanceNm.toFixed(1)} <small>MN</small></div></aside><aside className="conditions-card"><div className="conditions-title"><div><span className="live-dot" /> CONDICIONES</div><button title="Actualizar condiciones" onClick={() => void refreshConditions(voyage.lat, voyage.lon)}>↻</button></div><dl><div><dt>Viento</dt><dd>{conditions.windKn.toFixed(0)} kn · {compassPoint(conditions.windDirection)}</dd></div><div><dt>Olas</dt><dd>{conditions.waveHeight.toFixed(1)} m</dd></div><div><dt>Corriente</dt><dd>{conditions.currentKn.toFixed(1)} kn · {compassPoint(conditions.currentDirection)}</dd></div><div><dt>Prof. est.</dt><dd>~{conditions.depthM} m</dd></div></dl><small>{conditionsBusy ? "Actualizando…" : conditions.source === "live" ? "Datos meteorológicos en vivo" : "Datos estimados · profundidad de juego"}</small></aside>
