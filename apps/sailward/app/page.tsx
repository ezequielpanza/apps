...
export default function Home() {
  ...
  useEffect(() => { if (!toolsOpen) return; const toolsKey = "sailward.inventoryFolderTools.v1"; const inventory = document.querySelector<HTMLElement>(".tools-inventory"); if (!inventory) return; const baseTools = [...inventory.querySelectorAll<HTMLButtonElement>("section:not(.inventory-personal) > div > button")]; const folders = [...inventory.querySelectorAll<HTMLButtonElement>(".inventory-folder")]; const label = (button: HTMLButtonElement) => button.textContent?.replace("📁", "").replace("×", "").trim() ?? ""; const updateFolder = (button: HTMLButtonElement) => { try { const entries = JSON.parse(window.localStorage.getItem(toolsKey) ?? "{}") as Record<string, string[]>; const tools = entries[label(button)] ?? []; button.title = tools.length ? `${tools.length} herramienta${tools.length === 1 ? "" : "s"} asignada${tools.length === 1 ? "" : "s"}` : "Soltá herramientas aquí"; button.dataset.toolCount = String(tools.length); } catch { button.dataset.toolCount = "0"; } }; baseTools.forEach((button) => { button.draggable = true; button.addEventListener("dragstart", (event) => { event.dataTransfer?.setData("text/plain", button.textContent?.replace("▸", "").trim() ?? ""); event.dataTransfer?.setData("application/x-sailward-tool", button.getAttribute("data-floating-panel") ?? button.textContent ?? ""); }); }); folders.forEach((folder) => { updateFolder(folder); folder.addEventListener("dragover", (event) => { event.preventDefault(); folder.classList.add("is-drop-target"); }); folder.addEventListener("dragleave", () => folder.classList.remove("is-drop-target")); folder.addEventListener("drop", (event) => { event.preventDefault(); folder.classList.remove("is-drop-target"); const tool = event.dataTransfer?.getData("text/plain")?.trim(); if (!tool) return; const entries = JSON.parse(window.localStorage.getItem(toolsKey) ?? "{}") as Record<string, string[]>; const name = label(folder); entries[name] = [...new Set([...(entries[name] ?? []), tool])]; window.localStorage.setItem(toolsKey, JSON.stringify(entries)); updateFolder(folder); }); }); }, [toolsOpen, inventoryFolders]);
