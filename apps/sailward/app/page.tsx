...
export default function Home() {
  ...
  const [sailsPanelOpen, setSailsPanelOpen] = useState(true); const [helmPanelOpen, setHelmPanelOpen] = useState(true);
  ...
  useEffect(() => { const timer = window.setTimeout(() => { const timestamp = Date.now(); setNow(timestamp); try { const raw = window.localStorage.getItem(STORAGE_KEY) ?? window.localStorage.getItem(LEGACY_STORAGE_KEY); if (raw) { const restored = advanceVoyage(normalizeVoyage(JSON.parse(raw) as Partial<Voyage>), conditionsRef.current, timestamp); voyageRef.current = restored; setVoyage(restored); setSelectedPortId(restored.portId); } } catch { window.localStorage.removeItem(STORAGE_KEY); } try { const raw = window.localStorage.getItem(KEY_BINDINGS_STORAGE_KEY); if (raw) { const saved = JSON.parse(raw) as Partial<KeyBindings>; if (typeof saved.rudderLeft === "string" && typeof saved.rudderRight === "string") setKeyBindings({ rudderLeft: saved.rudderLeft, rudderRight: saved.rudderRight }); } } catch { window.localStorage.removeItem(KEY_BINDINGS_STORAGE_KEY); } setHydrated(true); }, 0); return () => window.clearTimeout(timer); }, []);
