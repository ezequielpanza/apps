(() => {
  if (window.__wanderLookDirectionContext) return;
  window.__wanderLookDirectionContext = true;
  ...
  function delta(from, to) { return ((normalize(to) - normalize(from) + 540) % 360) - 180; }
  ...
  function publish(event) {
    ...
    if (!isPhoneRaised(event)) {
      verticalSince = null;
      window.WanderLookDirection = { active: false, reason: 'phone_not_vertical', updated_at: new Date().toISOString() };
      document.dispatchEvent(new CustomEvent('wander:look-direction', { detail: window.WanderLookDirection }));
      return;
    }
    ...
    window.WanderLookDirection = {
      ...
      meaning: 'user_is_probably_pointing_phone_this_way',
