(() => {
  ...
  $('#simulation-toggle')?.addEventListener('change', (e) => setEnabled(e.target.checked));
  const joystick = $('#simulation-joystick-base');
  joystick?.addEventListener('pointerdown', (e) => { if (!enabled) return; pointerId = e.pointerId; joystick.setPointerCapture?.(e.pointerId); moveJoystick(e.clientX, e.clientY); });
  joystick?.addEventListener('pointermove', (e) => { if (enabled && e.pointerId === pointerId) moveJoystick(e.clientX, e.clientY); });
  joystick?.addEventListener('pointerup', release);
  joystick?.addEventListener('pointercancel', release);
  joystick?.addEventListener('lostpointercapture', release);

  window.WanderSimulator = { enable: () => setEnabled(true), disable: () => setEnabled(false), setEnabled, isEnabled: () => enabled, stop: stopMotion };
