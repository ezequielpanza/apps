(() => {
  ...
  const directionBearing = { north: 0, northeast: 45, east: 90, southeast: 135, south: 180, southwest: 225, west: 270, northwest: 315 };
  ...
  const directionLabels = { north: 'norte', south: 'sur', east: 'este', west: 'oeste', northeast: 'noreste', northwest: 'noroeste', southeast: 'sureste', southwest: 'suroeste' };
  ...
  function publishSimulatedMotion(point, mode, moving = true) {
    ...
    const water = Boolean(window.WanderSimulationWaterMode);
    lastSimulatedMotion = {
      transport_mode: water ? 'boat' : 'walking_or_land',
      likely_boat: water,
      speed_mps: speedMps,
  ...
  function updatePositionWithoutForcedCenter(next, mode) {
    try { window.WanderRevealMarker?.(); marker.setLatLng(next); } catch {}
  ...
  function restartTimer() { if (timer) window.clearInterval(timer); timer = window.setInterval(moveOneTick, TICK_MS); }
  ...
  function startOrAdjust(button) {
    ...
    if (timer && activeDirection === direction) modeIndex = Math.min(modeIndex + 1, modes.length - 1);
    else if (timer && oppositeDirections[activeDirection] === direction) {
      if (modeIndex > 0) { modeIndex -= 1; updateUi(); return; }
  ...
  manualLocationButton?.addEventListener('click', () => { window.WanderSimulationActive = true; }, true);
  document.addEventListener('wander:simulation-closed', stopMovement);
...
(() => {
  if (!document.querySelector('script[data-movement-overlay]')) {
    ...
    script.src = 'movement-overlay.js?v=20260701-6';
