(() => {
  ...
  async function requestRemote(method = 'GET', body = null) {
    ...
    const options = {
      ...
      headers: {
        ...
        'x-wander-device-key': currentIdentity.deviceKey,
      },
      cache: 'no-store',
    };
    if (body !== null) {
  ...
  function restoreBackup(backup) {
    ...
    localStorage.setItem(UPDATED_KEY, backup.clientUpdatedAt || restoredAt);
    if (backup.contentHash) localStorage.setItem(HASH_KEY, String(backup.contentHash));
    localStorage.setItem(RESTORE_KEY, restoredAt);
    setState({
      status: 'restoring',
      lastRestoreAt: restoredAt,
      lastSyncAt: backup.storedAt || backup.clientUpdatedAt || restoredAt,
      remoteExists: true,
      error: null,
    });
  ...
  async function uploadBackup(backup, options = {}) {
    ...
    const syncedAt = result.storedAt || new Date().toISOString();
    localStorage.setItem(UPDATED_KEY, backup.clientUpdatedAt);
    localStorage.setItem(HASH_KEY, backup.contentHash);
    setState({ status: 'synced', lastSyncAt: syncedAt, remoteExists: true, error: null });
    return result;
  }
    ...
    try {
      ...
      if (remote && localMeaningful) {
        ...
        if (remote.contentHash === localHash) {
          localStorage.setItem(UPDATED_KEY, remote.clientUpdatedAt || localUpdatedAt || new Date().toISOString());
          localStorage.setItem(HASH_KEY, localHash);
          setState({ status: 'synced', lastSyncAt: remote.storedAt || remote.clientUpdatedAt || null, remoteExists: true, error: null });
          return { synced: true };
        }
      }
      ...
      setState({ status: 'empty', remoteExists: false, error: null });
      return { empty: true };
    } catch (error) {
  ...
  async function synchronize(options = {}) {
    ...
    activeRequest = (async () => {
      try {
        ...
        const backup = await buildBackup({ clientUpdatedAt: options.touch === false ? undefined : new Date().toISOString() });
  ...
  function installSettings() {
    ...
    settingsCard.innerHTML = `
      ...
      <p class="panel-note">Puntos, recorridos y Bitácora se guardan automáticamente en Cloudflare. No usa cuenta: la copia pertenece a este teléfono y a la firma de esta APK.</p>
      ...
      <div class="cloud-backup-status-row"><span>Última copia</span><strong data-cloud-backup-last>—</strong></div>
      <div class="button-row compact-actions screen-card-actions"><button type="button" data-cloud-backup-sync><svg class="button-icon"><use href="wander-icons.svg#refresh"></use></svg><span>Sincronizar ahora</span></button></div>
      <p class="panel-note" data-cloud-backup-error hidden></p>
    `;
    document.querySelector('#settings-panel').appendChild(settingsCard);
    settingsCard.querySelector('[data-cloud-backup-sync]')?.addEventListener('click', () => synchronize({ force: true }));
    renderSettings();
  }
  ...
  function renderSettings() {
    ...
    if (last) last.textContent = formatDate(state.lastSyncAt);
    if (error) {
  ...
  function start() {
    ...
    window.addEventListener('online', () => synchronize({ touch: false }));
    ...
    periodicTimer = setInterval(() => synchronize({ touch: false }), PERIODIC_SYNC_MS);
    synchronize({ touch: false });
  }
  ...
  window.WanderCloudBackup = Object.freeze({
    ...
    constants: { STORAGE_KEYS, UPDATED_KEY, HASH_KEY, RESTORE_KEY },
  });

  setState({ lastRestoreAt: readStoredTimestamp(RESTORE_KEY) });
})();
