...
assert.match(plugin, /MAX_FALLBACK_DEPTH = 4/);
assert.match(plugin, /WARM_ANCESTOR_DEPTH = 2/);
...
assert.match(plugin, /findCachedAncestor/);
assert.match(plugin, /fallbackTileResponse/);
assert.match(plugin, /warmAncestorTiles/);
assert.match(plugin, /No cached tile and no validated network/);
assert.match(plugin, /result\.put\("zoomFallback", true\)/);
...
assert.match(mapCore, /const sourceSize = 256 \/ scale/);
...
console.log('PASS Wander restores exact cached tiles and crops cached parent tiles when offline zoom requests a missing level');
