...
assert.match(plugin, /MAX_TILE_COUNT = 4000/);
...
assert.match(plugin, /tileResponse\(readBytes\(file\), true, true/);
...
assert.match(mapCore, /nativeTiles\?\.getTile/);
...
assert.match(settings, /La descarga anticipada de regiones requiere una fuente OSM propia/);
...
console.log('PASS Wander starts locally, records routes without tiles and stores viewed OSM tiles on-device');
