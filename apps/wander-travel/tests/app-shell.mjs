...
assert.match(offlineTilePlugin, /OSM_TILE_TEMPLATE/);
assert.match(offlineTilePlugin, /ESRI_TILE_TEMPLATE/);
assert.match(offlineTilePlugin, /networkAvailable\(\)/);
assert.match(offlineTilePlugin, /tileResponse\(bytes, true, !fresh, source/);
...
console.log(`PASS Wander Web ${versionMatch[1]} / APK ${androidVersion.versionName} restores cloud backups, cached maps, and live compass orientation offline`);
