(() => {
  ...
  function plans() {
    return (log()?.listPlans?.() || []).map((item) => ({
      id: item.id,
      type: 'plan',
      day: item.day || localDay(item.scheduledAt || Date.now()),
      at: atValue(item.scheduledAt || Date.now()),
  ...
  async function buildDays() {
    ...
    const mergedTracks = [...dbTracks.filter((track) => track.status !== 'deleted')];
    const seen = new Set(mergedTracks.map((track) => track.id));
    sessionTracks().forEach((track) => { if (!seen.has(track.id)) mergedTracks.push(track); });
    const episodeMap = new Map(dbEpisodes.map((episode) => [episode.id, episode]));
    ...
    const ensureEpisode = (dayNode, id, fallbackAt) => {
      if (!dayNode.episodes.has(id)) {
        ...
        dayNode.episodes.set(id, {
          id,
          title: stored?.title || 'Episodio',
          startedAt: stored?.startedAt || fallbackAt,
          activities: new Map(),
        });
      }
  ...
  function dayMarkup(node) {
    ...
    const episodeMarkup = episodes.map((episode) => {
      const activities = [...episode.activities.entries()].map(([activity, tracks]) =>
        `<details class="utl-folder utl-activity" open><summary><span>▱</span><strong>${esc(activityLabel(activity))}</strong><small>${tracks.length} track${tracks.length === 1 ? '' : 's'}</small></summary><div>${tracks.sort((a, b) => a.startedAt - b.startedAt).map(itemMarkup).join('')}</div></details>`
      ).join('');
    ...
    const extrasMarkup = extras.length
      ? `<details class="utl-folder utl-activity" open><summary><span>▱</span><strong>Eventos, notas y planes</strong><small>${extras.length}</small></summary><div>${extras.map(itemMarkup).join('')}</div></details>`
      : '';
    const empty = !episodeMarkup && !extrasMarkup
      ? '<div class="utl-empty">Sin actividad registrada todavía.</div>'
      : '';
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .utl-item{display:grid;grid-template-columns:14px 1fr;gap:8px;margin:6px;padding:9px;border-radius:9px;background:rgba(2,6,23,.42)}.utl-item p,.utl-item small{margin:3px 0 0;display:block}.utl-item p{font-size:12px}.utl-item small{font-size:11px;opacity:.68}.utl-dot{width:10px;height:10px;border-radius:50%;margin-top:4px;background:#14b8a6}.utl-dot.relevance-suspect{background:#f59e0b}.utl-dot.relevance-irrelevant{background:#ef4444}.utl-symbol{opacity:.8}.utl-empty{padding:12px 11px 14px;opacity:.65;font-size:12px}
    `;
    ...
    return JSON.stringify({
      ...
      days: days.map((day) => [
        ...
        [...day.episodes.values()].map((episode) => [
          episode.id,
          [...episode.activities.entries()].map(([activity, tracks]) => [
            activity,
            tracks.map((track) => [track.id, track.status, Math.round(Number(track.distanceM || 0)), Number(track.endedAt || 0)]),
          ]),
        ]),
        day.loose.length,
        day.plans.length,
  ...
  window.addEventListener('wander:travel-log-change', render);
  window.addEventListener('wander:screen-change', (event) => {
    if (event.detail?.to === 'travel-log') setTimeout(render, 40);
  });
  ...
  window.WanderUnifiedTravelLog = Object.freeze({
    render,
    setFilter: () => { lastSignature = ''; render(); },
    queueEnrichment,
  });
  start();
})();
