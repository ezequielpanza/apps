(() => {
  ...
  const dirs = {
    ...
    west: [0, -1, 270, 'oeste'],
    east: [0, 1, 90, 'este'],
    southwest: [-0.707, -0.707, 225, 'suroeste'],
    south: [-1, 0, 180, 'sur'],
    southeast: [-0.707, 0.707, 135, 'sureste']
  };
  function setStatus(value) { const item = $('#simulator-status'); if (item) item.textContent = value; }
  function stop() {
  ...
  function tick() {
    ...
    const next = L.latLng(current.lat + meters * dir[0] / 111320, current.lng + meters * dir[1] / (111320 * Math.max(0.15, Math.cos(current.lat * Math.PI / 180))));
    base.revealMarker?.();
  ...
  $('#manual-location-button')?.addEventListener('click', () => { base.revealMarker?.(); stop(); });
