...
function changeSelectedPerson(delta){const current=PEOPLE.indexOf(selectedPerson),next=(current+delta+PEOPLE.length)%PEOPLE.length;selectedPerson=PEOPLE[next];try{localStorage.setItem(PERSON_STORAGE_KEY,selectedPerson)}catch{}const grid=$('peopleGrid');if(grid)grid.scrollTo({left:next*grid.clientWidth,behavior:'smooth'})}
...
function percentage(value,target){return target?value/target*100:0}
function progress(value,target){return clamp(percentage(value,target),0,100)}
...
window.addEventListener('DOMContentLoaded',()=>{renderSelectedDate();$('refreshBtn').addEventListener('click',loadAll);$('previousDayBtn').addEventListener('click',()=>changeDate(-1));$('nextDayBtn').addEventListener('click',()=>changeDate(1));$('todayBtn').addEventListener('click',()=>{selectedDate=localToday();renderSelectedDate();loadAll()});$('previousPersonBtn')?.addEventListener('click',()=>changeSelectedPerson(-1));$('nextPersonBtn')?.addEventListener('click',()=>changeSelectedPerson(1));loadAll()});
