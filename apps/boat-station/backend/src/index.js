  ...
  async fetch(request) {
    ...
    if (url.pathname.endsWith('/commands')) {
      if (request.method === 'POST') {
        ...
        const allowed = ['system.refresh','gps.new','gps.resume','gps.stop','gps.clear','gps.import','battery.scan','battery.add','battery.remove','battery.renameBank','battery.edit'];
        if (!allowed.includes(command)) return json({error:'invalid command'},400);
