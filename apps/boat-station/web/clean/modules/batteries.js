export function createBatteriesModule(requestRender,openManager){
  ...
  delete state.historyDays;
  if(!HISTORY_HOUR_LEVELS.includes(Number(state.historyHours)))state.historyHours=168;
  ...
  function historyRangeLabel(){
    const h=Number(state.historyHours);
  ...
  function historyPoints(){
    const cutoff=Date.now()-Number(state.historyHours)*3600000;
  ...
  function drawChart(root){
    ...
    const start=Date.now()-Number(state.historyHours)*3600000,end=Date.now(),span=Math.max(1,end-start);
  ...
  function changeHistoryZoom(direction,root){
    const i=HISTORY_HOUR_LEVELS.indexOf(Number(state.historyHours));
    const next=direction==='out'?Math.min(HISTORY_HOUR_LEVELS.length-1,i+1):Math.max(0,i-1);
    if(next===i)return;
    state.historyHours=HISTORY_HOUR_LEVELS[next];save();updateHistoryLabel(root);drawChart(root);
  }
