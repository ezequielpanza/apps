...
export function createPageLayoutEngine(cards){
  ...
  function hasVerticalClip(el){
    ...
    if(el.tagName==='CANVAS'||el.tagName==='SVG'||isLeafTextElement(el))return false;
  ...
  function contentFits(content,height){
    ...
    const root=content.getBoundingClientRect(),topLimit=root.top-FIT_EPSILON,bottomLimit=root.bottom+FIT_EPSILON;
  ...
  function minimumContentHeight(content){
    ...
    let low=DEFAULT_MIN_CONTENT_HEIGHT,high=Math.max(low,naturalContentHeight(content),160);
    ...
    if(contentFits(content,high)){while(high-low>1){const mid=(low+high)/2;if(contentFits(content,mid))high=mid;else low=mid}}
  ...
  function resetModule(card){
    if(!card?.dataset?.id)return false;
    const id=card.dataset.id,pages=[...card.querySelectorAll('.page[data-page]')];
    ensureScope();state.contentHeights[id]={};
    for(const pageEl of pages){
      const page=Number(pageEl.dataset.page),content=pageEl.querySelector(':scope > .page-content');
      if(!Number.isFinite(page)||!content)continue;
      state.contentHeights[id][page]=defaultContentHeight(content);
    }
    persist();
    applyContentHeight(card,currentPage(card),savedContentHeight(id,currentPage(card)));
    window.dispatchEvent(new CustomEvent('boatstation-module-size-reset',{detail:{id,card}}));
    return true;
  }
  ...
  function validateCard(card){if(!card||!card.isConnected||card.classList.contains('collapsed')||(resize&&resize.card===card))return;const page=currentPage(card),content=contentElement(card,page);if(!content)return;const current=parseFloat(card.style.getPropertyValue('--page-content-height'));const wanted=Number.isFinite(current)?current:savedContentHeight(card.dataset.id,page);applyContentHeight(card,page,wanted)}
  ...
  function mountCard(card){if(!card)return;const page=currentPage(card),track=card.querySelector('.track');if(track){track.style.transition='none';track.style.transform=`translate3d(-${page*100}%,0,0)`;track.getBoundingClientRect();track.style.transition=''}updatePager(card,page);applyContentHeight(card,page)}
  function mountAll(){cards?.querySelectorAll('.card').forEach(mountCard)}
  function setPage(card,page,{animate=true}={}){if(!card)return 0;const id=card.dataset.id,count=pageCount(card);if(!id||!count)return 0;page=Math.max(0,Math.min(count-1,Number(page)||0));ensureScope();state.pages[id]=page;persist();const track=card.querySelector('.track');if(track){track.style.transition=animate?'':'none';track.style.transform=`translate3d(-${page*100}%,0,0)`;if(!animate){track.getBoundingClientRect();track.style.transition=''}}updatePager(card,page);applyContentHeight(card,page);window.dispatchEvent(new CustomEvent('boatstation-page-change',{detail:{id,page,card}}));return page}
  function refreshPage(card,page=currentPage(card)){if(!card)return;updatePager(card,page);applyContentHeight(card,page,savedContentHeight(card.dataset.id,page))}
  function maxContentHeight(){return Math.max(1200,window.innerHeight*1.75)}
  function finishResize(event){if(!resize||event.pointerId!==resize.pointerId)return false;const current=resize;resize=null;current.card.classList.remove('resizing');const height=parseFloat(current.card.style.getPropertyValue('--page-content-height'));if(Number.isFinite(height))saveContentHeight(current.id,current.page,height);queueValidation(current.card);window.dispatchEvent(new CustomEvent('boatstation-page-resize-end',{detail:{id:current.id,page:current.page,height}}));return true}

  cards?.addEventListener('pointerdown',event=>{const handle=event.target.closest('.resize-handle');if(!handle||event.isPrimary===false||(event.pointerType==='mouse'&&event.button!==0))return;const card=handle.closest('.card');if(!card||card.classList.contains('collapsed'))return;const page=currentPage(card),content=contentElement(card,page);if(!content)return;event.preventDefault();event.stopImmediatePropagation();const startH=applyContentHeight(card,page);resize={pointerId:event.pointerId,card,id:card.dataset.id,page,startY:event.clientY,startH,minH:minimumContentHeight(content)};card.classList.add('resizing');window.dispatchEvent(new CustomEvent('boatstation-page-resize-start',{detail:{id:resize.id,page}}));try{handle.setPointerCapture(event.pointerId)}catch{}},{capture:true});
  cards?.addEventListener('pointermove',event=>{if(!resize||event.pointerId!==resize.pointerId)return;event.preventDefault();event.stopImmediatePropagation();const next=Math.max(resize.minH,Math.min(maxContentHeight(),resize.startH+(event.clientY-resize.startY)));resize.card.style.setProperty('--page-content-height',`${Math.round(next)}px`)},{capture:true,passive:false});
