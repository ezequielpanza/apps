(function(){
  // The native Core currently opens the PWA with ?mode=station. Detect that
  // synchronously so the Remote gate can never cover the Station UI.
  ...
  const gate=document.createElement('div');gate.id='bsRemoteGate';gate.innerHTML=`<div class="gate-card"><img class="gate-logo" src="boat_station_logo.png" onerror="this.src='icon.png'" alt="Boat Station"><h1>Boat Station</h1><p>Vinculá este dispositivo con la Boat Station del barco para ver sus módulos y datos.</p><button class="gate-btn primary" id="bsScanQr">Escanear QR</button><button class="gate-btn" id="bsManualToggle">Ingresar código</button><div class="manual" id="bsManualBox"><input id="bsManualCode" maxlength="19" inputmode="text" autocomplete="one-time-code" placeholder="XXXX-XXXX-XXXX-XXXX"><button class="gate-btn primary" id="bsManualConnect">Vincular</button><div class="hint">Código de 16 caracteres mostrado por Boat Station Local</div></div><video id="bsQrVideo" playsinline></video><div class="status" id="bsPairStatus"></div><a class="gate-link" id="bsApkDownload" href="/BoatStation.apk" download>Descargar APK de Boat Station</a></div>`;document.body.appendChild(gate);
  const status=gate.querySelector('#bsPairStatus'),manual=gate.querySelector('#bsManualBox'),input=gate.querySelector('#bsManualCode'),video=gate.querySelector('#bsQrVideo');let stream=null;
  ...
  function normalizeCode(v){return String(v||'').toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,16)}function prettyCode(v){const s=normalizeCode(v);return s.match(/.{1,4}/g)?.join('-')||''}input.addEventListener('input',()=>{input.value=prettyCode(input.value)});
  function savePairing(payload){try{let data=payload;if(typeof data==='string'){const t=data.trim();data=t.startsWith('{')?JSON.parse(t):{manualCode:normalizeCode(t)}}localStorage.setItem('bs.remote.pairing',JSON.stringify(data));if(data.stationId)localStorage.setItem('bs.stationId',data.stationId);if(data.token)localStorage.setItem('bs.pairingToken',data.token);if(data.boatName)localStorage.setItem('bs.boatName',data.boatName);localStorage.setItem('bs.remote.paired','1');status.textContent='Vinculación guardada';stopCamera();gate.remove();window.dispatchEvent(new CustomEvent('boatstation-remote-paired',{detail:data}))}catch(e){status.textContent='No se pudo interpretar la vinculación'}}
  gate.querySelector('#bsManualToggle').onclick=()=>{manual.classList.toggle('open');if(manual.classList.contains('open'))input.focus()};gate.querySelector('#bsManualConnect').onclick=()=>{const code=normalizeCode(input.value);if(code.length!==16){status.textContent='Ingresá los 16 caracteres';return}savePairing({type:'boat-station-manual',manualCode:code})};
  async function stopCamera(){if(stream){stream.getTracks().forEach(t=>t.stop());stream=null}video.srcObject=null;video.classList.remove('open')}async function scanLoop(detector){if(!stream)return;try{const codes=await detector.detect(video);if(codes&&codes.length){savePairing(codes[0].rawValue||'');return}}catch(e){}requestAnimationFrame(()=>scanLoop(detector))}
  gate.querySelector('#bsScanQr').onclick=async()=>{if(!navigator.mediaDevices?.getUserMedia){status.textContent='La cámara no está disponible en este navegador';return}if(!('BarcodeDetector'in window)){status.textContent='Este navegador no admite lectura QR directa. Usá el código manual.';manual.classList.add('open');input.focus();return}try{stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'}}});video.srcObject=stream;video.classList.add('open');await video.play();status.textContent='Apuntá la cámara al QR de Boat Station Local';const detector=new BarcodeDetector({formats:['qr_code']});scanLoop(detector)}catch(e){status.textContent='No se pudo abrir la cámara'}};
