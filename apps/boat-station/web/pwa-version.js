(function(){
  ...
  function setRefreshUi(on,remote){
    ...
    indicator.querySelector('.label').textContent=remote?'Actualización remota':'Actualizando';
    indicator.classList.toggle('show',!!on);
    const host=document.getElementById('remoteFreshness');
    if(host){
      host.classList.toggle('refreshing',!!on);
      const text=host.querySelector('.remote-connection-text');
      if(on&&text)text.textContent=remote?'Actualización remota':'Actualizando';
      if(!on)window.BoatStationRemoteUI?.markUpdated?.(Date.now());
    }
  }

  async function fetchVersion(){
    try{const r=await fetch('./PWA_VERSION?v='+Date.now(),{cache:'no-store',headers:{'Cache-Control':'no-cache'}});if(r.ok)return (await r.text()).trim()}catch(_){}
    return '';
  }
  ...
  async function runSystemRefresh(options={}){
    ...
    const remote=options.source==='remote'||options.source==='remote-command'||(!isLocal&&options.sendCore!==false);
    refreshing=true;setRefreshUi(true,remote);
    try{
      ...
      if(latest)localVersion=latest;
      render();
      setRefreshUi(true,remote);
      setTimeout(()=>location.reload(),180);
      return true;
    }catch(_){
      setRefreshUi(false,remote);refreshing=false;return false;
    }
  }
  ...
  function wrapDataSync(){
    ...
    const sync=window.BoatStationDataSync;if(typeof sync.exportSnapshot!=='function')return;
    const original=sync.exportSnapshot.bind(sync);
    sync.exportSnapshot=function(){const snapshot=original()||{};snapshot.pwaVersion=localVersion;return snapshot};wrappedSync=true;
  }
  ...
  function wrapRemoteUi(){
    ...
    ui.scheduleData=function(snapshot,apply){if(snapshot&&typeof snapshot.pwaVersion==='string'&&snapshot.pwaVersion.trim())coreVersion=snapshot.pwaVersion.trim();render();return original(snapshot,apply)};
    wrappedRemote=true;render();
  }

  function installPullRefresh(){
    document.addEventListener('touchstart',e=>{
      if(refreshing||window.scrollY>1||e.touches.length!==1||e.target.closest('input,textarea,select,.sheet.open,.station-manager.open'))return;
      const t=e.touches[0];pull={x:t.clientX,y:t.clientY,armed:false};
    },{passive:true});
    document.addEventListener('touchmove',e=>{
      if(!pull||e.touches.length!==1)return;const t=e.touches[0],dx=t.clientX-pull.x,dy=t.clientY-pull.y;
      if(dy<0||Math.abs(dx)>Math.abs(dy)*.8){pull=null;return}
      pull.armed=dy>=82;
    },{passive:true});
