(function(){
  ...
  function apply(card){if(!card||card.classList.contains('collapsed'))return;const body=card.querySelector('.card-body');if(!body)return;const h=storedHeight(card.dataset.id,pageIndex(card));if(h!==null)setHeight(body,h)}
  ...
  document.addEventListener('pointermove',e=>{if(!drag||e.pointerId!==drag.pointerId)return;const max=Math.max(900,window.innerHeight*1.5),next=Math.max(70,Math.min(max,drag.startH+(e.clientY-drag.startY)));drag.lastH=next;requestAnimationFrame(()=>{if(drag?.body?.isConnected)setHeight(drag.body,next)})},false);
