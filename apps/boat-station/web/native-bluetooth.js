...
function removeConfiguredBattery(id){const nativeId=Number(id);if(!Number.isFinite(nativeId))return false;invalidateConfig();try{native()?.deleteBattery?.(nativeId)}catch(_){return false}invalidateConfig();setTimeout(()=>{savedBatteries(true);publishScan()},100);return true}
...
window.addEventListener('boatstation-core-ready',()=>{invalidateConfig();savedBatteries(true);exposeCoreAdapter();hookRemoteRemoval()});exposeCoreAdapter();
