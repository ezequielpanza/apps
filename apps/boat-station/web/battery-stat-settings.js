(function(){
  ...
  function selectorRows(){const order=selected();return FIELDS.map(id=>{const on=order.includes(id),pos=order.indexOf(id);return `<div class="add-module-row" data-stat-row="${esc(id)}"><span>${esc(id)}</span><span style="display:flex;gap:6px;align-items:center"><button type="button" class="fullscreen-back" data-stat-up="${esc(id)}" ${!on||pos===0?'disabled':''}>↑</button><button type="button" class="fullscreen-back" data-stat-down="${esc(id)}" ${!on||pos===order.length-1?'disabled':''}>↓</button><button type="button" class="add-module-state ${on?'present':''}" data-stat-toggle="${esc(id)}">${on?'✓':'+'}</button></span></div>`}).join('')}
  ...
  let queued=false;const queueRender=()=>{if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;renderStats()})};
