export function createBatteriesModule(requestRender,openManager){
  ...
  function axisLabel(time,hours){const d=new Date(time),hh=String(d.getHours()).padStart(2,'0'),mm=String(d.getMinutes()).padStart(2,'0'),day=String(d.getDate()).padStart(2,'0'),month=String(d.getMonth()+1).padStart(2,'0');if(hours<=12)return `${hh}:${mm}`;if(hours<=72)return `${day}/${month} ${hh}:${mm}`;return `${day}/${month}`}
  function chooseTickStep(spanMs,target){const nice=[5,15,30,60,120,180,360,720,1440,2880,4320,10080,20160,43200,86400,129600].map(m=>m*60000),raw=spanMs/Math.max(1,target-1);return nice.find(v=>v>=raw)||nice[nice.length-1]}
  function roundedTicks(start,end,target){const step=chooseTickStep(end-start,target),first=Math.ceil(start/step)*step,out=[];for(let t=first;t<=end;t+=step)out.push(t);return out}
  function drawChart(root){
    ...
    if(pts.some(Boolean)){const slot=r.width/12,bar=Math.max(1,slot*.7);pts.forEach((p,i)=>{if(!p)return;const h=Math.max(1,(Math.max(0,Math.min(100,Number(p.soc)||0))/100)*plotH),x=i*slot+(slot-bar)/2;c.fillStyle='#1ed7e5';c.fillRect(x,plotH-h,bar,h)})}
    const target=r.width<360?3:r.width<620?4:5,ticks=roundedTicks(start,end,target);c.font='10px system-ui, sans-serif';c.fillStyle='#7890a1';c.textBaseline='bottom';ticks.forEach((time,i)=>{const x=(time-start)/(end-start)*r.width,text=axisLabel(time,hours);c.textAlign=i===0&&x<18?'left':i===ticks.length-1&&r.width-x<18?'right':'center';c.fillText(text,x,r.height-2)});
  }
