export function createBatteriesModule(requestRender,openManager){
  ...
  function save(){try{const copy={...state,root:undefined,remoteAuthoritative:undefined,scanDevices:undefined};localStorage.setItem('bs.batteries.state',JSON.stringify(copy))}catch(_){}}
  function stats(){
  ...
  function aggregateForChart(points,start,end,maxBars){if(!points.length)return[];const span=Math.max(1,end-start),count=Math.max(1,Math.min(maxBars,points.length)),step=span/count,buckets=Array.from({length:count},()=>({sum:0,n:0,time:0}));for(const p of points){const i=Math.max(0,Math.min(count-1,Math.floor((Number(p.time)-start)/step))),b=buckets[i];b.sum+=Number(p.soc)||0;b.n++;b.time=Number(p.time)}return buckets.map((b,i)=>b.n?{time:b.time||start+(i+.5)*step,soc:b.sum/b.n}:null).filter(Boolean)}
  function axisLabel(time,hours){const d=new Date(time),hh=String(d.getHours()).padStart(2,'0'),mm=String(d.getMinutes()).padStart(2,'0'),day=String(d.getDate()).padStart(2,'0'),month=String(d.getMonth()+1).padStart(2,'0');if(hours<=12)return `${hh}:${mm}`;if(hours<=72)return `${day}/${month} ${hh}h`;return `${day}/${month}`}
  function drawChart(root){
    ...
    const hours=Number(state.historyHours),axisH=22,plotH=Math.max(20,r.height-axisH),end=Date.now(),start=end-hours*3600000;
    ...
    const pts=aggregateForChart(historyPoints(),start,end,Math.max(12,Math.floor(r.width/5)));
    if(pts.length){const slot=r.width/Math.max(1,pts.length),bar=Math.max(1,slot*.7);c.fillStyle='#1ed7e5';pts.forEach((p,i)=>{const h=Math.max(1,(Math.max(0,Math.min(100,Number(p.soc)||0))/100)*plotH),x=i*slot+(slot-bar)/2;c.fillRect(x,plotH-h,bar,h)})}
    const labelCount=r.width<360?3:r.width<620?4:5;c.font='10px system-ui, sans-serif';c.fillStyle='#7890a1';c.textBaseline='bottom';for(let i=0;i<labelCount;i++){const f=labelCount===1?0:i/(labelCount-1),time=start+(end-start)*f,text=axisLabel(time,hours),x=r.width*f;c.textAlign=i===0?'left':i===labelCount-1?'right':'center';c.fillText(text,x,r.height-2)}
  }
  ...
  function compactHistory(){const now=Date.now(),cutoff=now-90*86400000,src=state.history.filter(p=>Number(p.time)>=cutoff&&Number.isFinite(Number(p.soc))).sort((a,b)=>a.time-b.time);const out=[],bins=new Map();for(const p of src){const age=now-Number(p.time);let bucket=30000;if(age>14*86400000)bucket=7200000;else if(age>48*3600000)bucket=1800000;else if(age>6*3600000)bucket=300000;const key=Math.floor(Number(p.time)/bucket)*bucket,prev=bins.get(key);if(prev){prev.sum+=Number(p.soc);prev.n++;prev.time=Math.max(prev.time,Number(p.time))}else bins.set(key,{time:Number(p.time),sum:Number(p.soc),n:1})}for(const b of bins.values())out.push({time:b.time,soc:b.sum/b.n});state.history=out.sort((a,b)=>a.time-b.time)}
  function appendHistory(soc){const now=Date.now(),last=state.history[state.history.length-1];if(!last||now-last.time>30000)state.history.push({time:now,soc});if(state.history.length%120===0)compactHistory()}
  function localUpdateBattery(data){if(!data)return;const id=String(data.id||data.address||data.deviceId||data.mac||data.name||'battery');let b=state.batteries.find(x=>String(x.id)===id);if(!b){b={id,name:data.name||data.deviceName||'Batería',capacityAh:num(data.capacityAh)||0,connected:true};state.batteries.push(b)}Object.assign(b,data,{id,connected:data.connected!==false});const s=stats();if(s.soc!==null&&!state.remoteAuthoritative)appendHistory(s.soc);save();requestRender('batteries')}
  function updateBattery(data){localUpdateBattery(data)}
  function exportRemoteState(){compactHistory();return{bankName:String(state.bankName||'Banco principal'),batteries:clone(state.batteries),history:clone(state.history),scanDevices:clone(state.scanDevices)}}
  ...
  function localRemoveBattery(id){state.batteries=state.batteries.filter(b=>String(b.id)!==String(id));save();requestRender('batteries')}
  ...
  function localRenameBank(name){const n=String(name||'').trim();if(!n)return;state.bankName=n;save();requestRender('batteries')}
  ...
  function localEditBattery(payload){const id=String(payload?.id||''),b=state.batteries.find(x=>String(x.id)===id);if(!b)return;if(typeof payload.name==='string'&&payload.name.trim())b.name=payload.name.trim();if(Number.isFinite(Number(payload.capacityAh))&&Number(payload.capacityAh)>=0)b.capacityAh=Number(payload.capacityAh);save();requestRender('batteries')}
  ...
  return {id:'batteries',name:'Baterías',pages:3,summary,page,afterRender,state,updateBattery,addBattery,removeBattery,renameBank,editBattery,stats,exportRemoteState,applyRemoteState};
}
