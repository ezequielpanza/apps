...
export function createProjectTreeController({treeElement,menuElement,getTree,getSelectedId,setSelectedId,save,openEditor,openGameSectionEditor,openRoomSectionEditor,onCreateResource,onDeleteResource,getRoomSectionCount}){
  ...
  function activate(id){
    ...
    if(node.kind==='root'||node.kind==='folder'||node.itemType==='game'){
      setSelectedId(id);toggleNode(node);save();render();return;
    }
    if(node.kind==='game-section'){
      setSelectedId(id);
      if(node.itemType)toggleNode(node);
      save();render();
      if(node.sectionKey==='settings')openGameSectionEditor(id);
      return;
    }
  ...
  function startRename(id){
    requestAnimationFrame(()=>{
      ...
      input.addEventListener('keydown',event=>{if(event.key==='Enter')commit();if(event.key==='Escape'){done=true;render();}});input.addEventListener('blur',commit,{once:true});
    });
  }

  function nextName(type,gameId){
    let count=0;
    walk(tree(),node=>{if(node.kind==='item'&&node.itemType===type&&(type==='game'||node.gameId===gameId))count++;});
    return type==='game'?`Game ${count+1}`:`${TYPES[type].label} ${count+1}`;
  }

  function createEntry(targetId,type){
    const found=findNode(tree(),targetId);if(!found)return;
    if(found.node.kind==='root'&&found.node.id==='games'&&type==='game'){
      const id=uid('game'),item={id,kind:'item',label:nextName('game'),rootId:'games',itemType:'game',gameId:id,open:true,children:createGameSections(id)};
      found.node.children.push(item);found.node.open=true;onCreateResource(item);setSelectedId(item.id);save();render();return;
    }
    if(!['game-section','folder'].includes(found.node.kind))return;
    const gameId=found.node.gameId,sectionKey=found.node.sectionKey,itemType=found.node.itemType;
    if(!gameId||!sectionKey||!itemType)return;
    const target=found.node;
    if(type==='folder'){
      const folder={id:uid('folder'),kind:'folder',label:'New Folder',rootId:'games',gameId,sectionKey,itemType,open:false,children:[]};
      target.children.push(folder);target.open=true;setSelectedId(folder.id);save();render();return;
    }
    if(type!==itemType)return;
    const item={id:uid(type),kind:'item',label:nextName(type,gameId),rootId:'games',gameId,sectionKey,itemType:type};
    if(type==='room'){item.open=false;item.children=createRoomSections(item.id,gameId);}
    target.children.push(item);target.open=true;onCreateResource(item);setSelectedId(item.id);save();render();
  }

  function buildMenu(id){
    const found=findNode(tree(),id);if(!found)return'';
    if(found.node.kind==='root'&&found.node.id==='games')return`<div class="create-menu-label">Create in Games</div><button data-create-type="game"><span>◆</span><div><strong>Game</strong><small>Add a playable game</small></div></button>`;
    if(!['game-section','folder'].includes(found.node.kind)||!found.node.itemType)return'';
    const type=TYPES[found.node.itemType];
    return`<div class="create-menu-label">Create in ${esc(found.node.label)}</div><button data-create-type="${found.node.itemType}"><span>${type.icon}</span><div><strong>${esc(type.label)}</strong><small>Add to this game</small></div></button><div class="create-menu-separator"></div><button data-create-type="folder"><span>▱</span><div><strong>Subfolder</strong><small>Create a nested folder</small></div></button>`;
  }
  ...
  function canMoveMaster(dragged,target){
    if(!dragged||!target||!['folder','item'].includes(dragged.node.kind)||dragged.node.itemType==='game'||!['game-section','folder'].includes(target.node.kind))return false;
    if(dragged.node.gameId!==target.node.gameId||dragged.node.sectionKey!==target.node.sectionKey||dragged.node.itemType!==target.node.itemType)return false;
  ...
  function bind(){
    treeElement.addEventListener('click',event=>{const actionElement=event.target.closest('[data-action]');if(!actionElement)return;const{action,nodeId}=actionElement.dataset;if(action==='activate')activate(nodeId);if(action==='edit-name'){event.stopPropagation();startRename(nodeId);}if(action==='delete'){event.stopPropagation();deleteNode(nodeId);}if(action==='delete-reference'){event.stopPropagation();deleteReference(nodeId);}if(action==='create-menu'){event.stopPropagation();select(nodeId);openMenu(nodeId,actionElement);}});
